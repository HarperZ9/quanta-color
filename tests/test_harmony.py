...
class TestOutputRange:
    ...
    def test_complementary_range(self, base):
        for color in complementary(base):
            assert np.all(color >= 0.0) and np.all(color <= 1.0), f"complementary out of range: {color}"

    @pytest.mark.parametrize("base", [BASE_RED, BASE_BLUE, BASE_GREEN])
    def test_split_complementary_range(self, base):
        for color in split_complementary(base):
            assert np.all(color >= 0.0) and np.all(color <= 1.0), f"split_complementary out of range: {color}"

    @pytest.mark.parametrize("base", [BASE_RED, BASE_BLUE, BASE_GREEN])
    def test_triadic_range(self, base):
        for color in triadic(base):
            assert np.all(color >= 0.0) and np.all(color <= 1.0), f"triadic out of range: {color}"

    @pytest.mark.parametrize("base", [BASE_RED, BASE_BLUE, BASE_GREEN])
    def test_tetradic_range(self, base):
        for color in tetradic(base):
            assert np.all(color >= 0.0) and np.all(color <= 1.0), f"tetradic out of range: {color}"

    @pytest.mark.parametrize("base", [BASE_RED, BASE_BLUE, BASE_GREEN])
    def test_analogous_range(self, base):
        for color in analogous(base):
            assert np.all(color >= 0.0) and np.all(color <= 1.0), f"analogous out of range: {color}"

    @pytest.mark.parametrize("base", [BASE_RED, BASE_BLUE, BASE_GREEN])
    def test_monochromatic_range(self, base):
        for color in monochromatic(base):
            assert np.all(color >= 0.0) and np.all(color <= 1.0), f"monochromatic out of range: {color}"
...
class TestSchemeProperties:
    ...
    def test_complementary_colors_differ(self):
        ...
        result = complementary(BASE_RED)
        assert not np.allclose(result[0], result[1], atol=0.05), "Complementary colors should differ"
    ...
    def test_all_output_shapes(self):
        ...
        for scheme_name in SCHEMES:
            ...
            for i, color in enumerate(result):
                assert color.shape == (3,), f"{scheme_name} color {i} has shape {color.shape}"
