...
class LUTWorkshopPage(QWidget):
    ...
    def _export_lut(self, fmt: str):
        ...
        path, _ = QFileDialog.getSaveFileName(self, f"Export {fmt.upper()}", f"quanta_lut{default_ext}", ext_filter)
        if not path:
            return

        try:
            # Try library exporter first
            from quanta_color.lut_io import write_clf, write_cube
    ...
    def _write_cube_fallback(self, path: str):
        ...
        try:
            with open(path, "w") as f:
                f.write("# Generated by Quanta Color\n")
                f.write('TITLE "Quanta Color LUT"\n')
