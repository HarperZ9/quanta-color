...
the tone mapping operators from quanta_color.tonemap.
"""
...
from quanta_color.gui.widgets.tone_curve import ToneCurve
...
class LUTWorkshopPage(QWidget):
    ...
    def _load_operators(self):
        try:
            from quanta_color.tonemap import OPERATORS
            for name in OPERATORS.keys():
                self._op_combo.addItem(name)
        except Exception:
            for name in [
                "reinhard", "reinhard_extended", "aces", "aces_hill",
                "hable", "lottes", "uchimura", "agx",
                "pbr_neutral", "bt2390", "bt2446", "knee",
            ]:
                self._op_combo.addItem(name)
    ...
    def _generate_lut(self):
        ...
        self._status_label.setText(
            f"Generated {size}\u00b3 LUT ({total:,} entries) "
            f"using {op_name} operator."
        )
        self._update_preview()
    ...
    def _export_lut(self, fmt: str):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, f"Export {fmt.upper()}", f"quanta_lut{default_ext}", ext_filter
        )
        if not path:
            return
        ...
        try:
            ...
            from quanta_color.lut_io import write_cube, write_clf
            if fmt == "cube":
            ...
            if fmt == "cube":
                ...
                self._status_label.setText(
                    "CLF export requires the lut_io module. Try .cube instead."
                )
    ...
    def _write_cube_fallback(self, path: str):
        ...
        try:
            with open(path, "w") as f:
                f.write(f"# Generated by Quanta Color\n")
                f.write(f"TITLE \"Quanta Color LUT\"\n")
